...
Page({
  ...
  async loadDay() {
    ...
    try {
      ...
      if (res.success) {
        ...
        const activeModuleId = Number(this.data.activeModuleId || 0);
        const current = (activeModuleId ? modules.find(m => Number(m.id) === activeModuleId) : null)
          || modules.find(m => !m.progress.completed)
          || modules[0]
          || {};
        this.setData({
          day: res.data.day || {},
        ...
        wx.setNavigationBarTitle({ title: res.data.day.title || '今日 IELTS' });
      } else {
  ...
  openModuleTask(e) {
    ...
    if (!id) return;
    wx.navigateTo({
      url: `/pages/ielts/task/task?day=${this.data.dayNumber}&moduleId=${id}`
    });
  },
